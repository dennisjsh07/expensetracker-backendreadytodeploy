...
async function onSubmit(e){
    ...
    try{
        ...
        const response = await axios.post('http://16.16.204.15:3000/expense/add-expense',myObj,{headers: {'Authorization': token}});
...
async function getRequest(page){
    try{
        const token = localStorage.getItem('token');
        const response = await axios.get('http://16.16.204.15:3000/expense/get-expense', {
...
function showInUi(data){
    ...
    async function onDeleteClick(e){
        try{
            ...
            const token = localStorage.getItem('token');
            const response = await axios.delete(`http://16.16.204.15:3000/expense/delete-expense/${userId}`, {headers: {'Authorization': token}});
            console.log(response);
...
async function onBuyClick(e){
    try{
        ...
        const token = localStorage.getItem('token');
        const response = await axios.get('http://16.16.204.15:3000/purchase/premium-membership', {headers: {'Authorization': token}});
        console.log(response);
        ...
        var options = {
            ...
            'handler': async(response)=>{
                const token = localStorage.getItem('token');
                const res = await axios.post('http://16.16.204.15:3000/purchase/update-status',{
...
document.getElementById('downloadExpense').addEventListener('click', async(e)=>{
    try{
        const token = localStorage.getItem('token');
        const awsS3Data = await axios.get('http://16.16.204.15:3000/expense/download', {
            headers: {'Authorization': token},
        });
...
async function showAllDownloadsOfUser() {
    try {
        ...
        let downloads = await axios.get('http://16.16.204.15:3000/expense/alldownload', { headers: { Authorization: token } })
